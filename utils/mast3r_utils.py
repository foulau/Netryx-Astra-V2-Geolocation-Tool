...
# Search multiple locations for MASt3R
MAST3R_DIR = None
for _candidate in [
    os.path.join(PROJECT_ROOT, "mast3r"),           # ./mast3r/
    os.path.join(PROJECT_ROOT, "..", "mast3r"),      # ../mast3r/ (recommended)
    os.path.expanduser("~/mast3r"),                  # ~/mast3r/
    os.path.join(PROJECT_ROOT, "..", "..", "mast3r"), # ../../mast3r/
]:
    _candidate = os.path.abspath(_candidate)
    if os.path.exists(os.path.join(_candidate, "mast3r", "model.py")):
        MAST3R_DIR = _candidate
        break
...
if MAST3R_DIR is not None:
    ...
    # Add dust3r submodule to path
    _dust3r_dir = os.path.join(MAST3R_DIR, "dust3r")
    if os.path.exists(_dust3r_dir) and _dust3r_dir not in sys.path:
        sys.path.insert(0, _dust3r_dir)
    ...
    print(f"[MASt3R] Found MASt3R at: {MAST3R_DIR}")
else:
    ...
    try:
        ...
        if MAST3R_DIR is not None:
            ...
            if os.path.exists(weights_path):
                print(f"[MASt3R] Loading from local weights: {weights_path}")
        ...
        print("[MASt3R] Local weights not found, downloading from Hugging Face...")
        _mast3r_model = AsymmetricMASt3R.from_pretrained(
...
def get_mast3r_matches(img1_pil, img2_pil, model, image_size=512):
    ...
    with torch.no_grad():
        ...
        # Scale matches back to original image coordinates
